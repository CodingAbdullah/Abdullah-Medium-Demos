    ...
    if (data.length === 0) {
        ...
        try {
            ...
            if (comparison) {
                ...
                return Response.json({ error: "Invalid verification ID, password will not be reset" }, { status: 500 });
            }
        }
        catch (err) {
            ...
            if (error) {
                ...
                return Response.json({ error: "Token expired, deleted from EmailToken collection!" }, { status: 500 });
            }
        }
    }
}